...
all_bytes = list(map(parse_point, input))
...
def shortest_path(
    bytes: set[Point], bound_i: int, bound_j: int
) -> float | None:
...
left = 0
right = len(all_bytes) - 1

while left + 1 != right:
    mid = left + (right - left) // 2
    bytes: set[Point] = set(all_bytes[i] for i in range(mid))
    if shortest_path(bytes, bound_i, bound_j) is not None:
        left = mid
    else:
        right = mid

print(all_bytes[left])
