...
def seek(p1: Point, p2: Point, direction: Vector) -> list[Point]:
    result: list[Point] = []
    p = p1
    while True:
        ...
        if d1 == d2:
            continue
        elif d1 > d2 and 2 * d2 == d1:
            result.append(p)
        elif d2 > d1 and 2 * d1 == d2:
            result.append(p)
...
for _, locations in antennae.items():
    for p1, p2 in combinations(locations, 2):
        ...
        for p in seek(p1, p2, direction):
            unique_antinodes.add(p)
        for p in seek(p1, p2, flip(direction)):
            unique_antinodes.add(p)
